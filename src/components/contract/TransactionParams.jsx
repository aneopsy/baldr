...
class TransactionParams extends React.Component {
  ...
  updateNonce(address) {
    this.props.tx._parent.web3.eth.getTransactionCount(address).then(nonce => {
      this.setState({ nonce });
    });
  }
  ...
  createUnsignedTx() {
    const toAddress = this.props.tx._parent._address ? { to: this.props.tx._parent._address } : {};
        ...
        <Row {...rowLayout}>
          <Col {...colLayout}>
            ...
            >
              ...
              />
              <Button onClick={this.handleMetamaskButton} style={{ margin: '8px 0 0 0' }}>
                Metamask
              </Button>
            </Form.Item>
          </Col>
