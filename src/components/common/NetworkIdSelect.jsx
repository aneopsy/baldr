...
class NetworkIdSelect extends React.Component {
  ...
  render() {
    ...
    // const selectPresets = this.idPresets.map(item => <Option value={item.id}>{item.label}</Option>);
    const selectAfter = (
      <Select defaultValue={selectedValue} onSelect={this.handleIdSelect} className={styles.idList}>
