...
export const getNodeInfo = (nodeKey) => {
  const nodeList = getNetworkList();
  let network = nodeList.find(
    (network) => network.nodes.find((node) => node.key === nodeKey) != undefined
  );
...
export const existCustomNode = function (nodeList, nodeName) {
  ...
  return (
    nodeList
      .find((item) => item.type === customNetworkType)
      .nodes.find((node) => node.key === key) != undefined
  );
};
...
export const addCustomNode = function (
  ...
  storage.saveNetworks(
    nodeList.find((item) => item.type === customNetworkType)
  );
  return nodeList;
};
